...
// 第3分岐(`::`が先頭以外にある形)は、`::`前後どちらにも複数セグメントが
// 置かれ得る(例: `2001:db8::1:2:192.0.2.1`)。実在するIPv6の仕様上は前後
// 合計で最大6セグメントまでだが、スクラバーとしては誤って過剰マッチする方が
// 取りこぼして情報が漏れるより安全なため、上限チェックは行わず緩めに許容する。
const IPV4_MAPPED_IPV6_PATTERN = new RegExp(
  "(?<![:\\w.])(?:" +
    ...
    "|" +
    `(?:${IPV6_SEGMENT}:){1,5}:(?:${IPV6_SEGMENT}:){0,4}${IPV4_PART}` +
    ")(?![:\\w.])",
  "g"
);
